 function AccordionMenu(options) {
	this.config = {
		containerCls        : '.wrap-menu-left',                // 外层容器
		menuArrs            :  '',                         //  JSON传进来的数据
 ...
 AccordionMenu.prototype = {
	...
	_createSubMenu: function(children,lihtml){
		...
		$(children).each(function(index,item){
			...
			subLi = $('<li><a href="'+(url.indexOf('/') == 0 ? WEB_ROOT + url : url)+'">'+item.name+'</a></li>');
			if(item.children && item.children.length > 0) {

				$(subLi).children('a').prepend('<img src="' + WEB_ROOT + '/images/blank.gif" alt=""/>');
