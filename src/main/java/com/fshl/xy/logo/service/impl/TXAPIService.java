...
public class TXAPIService {
    ...
    public void crawlGupiao() {
        ...
        if (gupiaos.size() > 100) {
            ...
            List<Gupiao> currGupiaos = new ArrayList<>();
            for(int i = 0; i < gupiaos.size(); i++){
                currGupiaos.add(gupiaos.get(i));
                if (i % 10 == 0) {
                    try {
                        gupiaoServiceImpl.insertBatch(currGupiaos);
                    } catch (Exception e) {
                        LOG.error(currGupiaos + ", errorMsg:" + e.getMessage(), e);
                    }finally {
                        currGupiaos.clear();
                    }
                }
    ...
    public void crawlZijin() {
        ...
        if (zijins.size() > 100) {
            ...
            List<Zijin> currZijins = new ArrayList<>();
            for (int i = 0; i < zijins.size(); i++) {
                currZijins.add(zijins.get(i));
                if (i % 10 == 0) {
                    try {
                        zijinServiceImpl.insertBatch(currZijins);
                    } catch (Exception e) {
                        LOG.error(currZijins + ", errorMsg:" + e.getMessage(), e);
                    }finally {
                        currZijins.clear();
                    }
                }
